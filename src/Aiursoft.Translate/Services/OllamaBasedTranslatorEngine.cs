...
public class OllamaBasedTranslatorEngine(
    ...
    MarkdownShredder shredder) : IScopedDependency
{
    ...
    {
        var targetLanguage = LocalizationPlugin.SupportedCultures.TryGetValue(language, out var fullName)
            ? $"{language}, {fullName}"
            : language;
        ...
        CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation(
            ...
            trimmedSource, options.Value.CompletionApiUrl, options.Value.Model);
        var aiResponseRaw = await retryEngine.RunWithRetry(async _ =>
        {
            var result = await chatClient.AskModel(content, options.Value.CompletionApiUrl, options.Value.Token,
                cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        logger.LogInformation(@"Calling Ollama to translate: ""{trimmedWord}"" to ""{lang}"", Instance: ""{instance}"", Model: ""{model}""",
            trimmedWord, language, options.Value.CompletionApiUrl, options.Value.Model);
        var aiResponseRaw = await retryEngine.RunWithRetry(async _ =>
        {
            var result = await chatClient.AskModel(content, options.Value.CompletionApiUrl, options.Value.Token, cancellationToken);
            return ExtractTranslation(result.GetAnswerPart());
        }, attempts: 5);
